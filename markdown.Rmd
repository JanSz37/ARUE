...
* city: City where the apartment is located. We set this parameter to "warszawa" to indicate that we are only interested in Warsaw.
...
Size and Layout:
* squareMeters: Size of the apartment in square meters. It will be an important factor in modeling, as we intend to divide price with it.
...
* latitude & longitude: Geographical coordinates. Key factors in our analysis, it is the basis for calculating centroid scores later.
* centreDistance: Distance from city center. Also an important factor.
* poiCount: Number of points of interest nearby.
* Distances to various amenities (all in kilometers, with some missing values): schoolDistance, clinicDistance, postOfficeDistance, kindergartenDistance, restaurantDistance, collegeDistance, pharmacyDistance. They will be important in evaluating centroid score significance in the model.
...
The above graphs illustrate the points on a Warsaw shaped map. Important to note are the transport points not belonging to Warsaw. The city bus service provides tickets for a number of local transporters, encorporating them into their infrastructure. Those points are generaly not considered by us, unless a property in Warsaw lies on the very edge of the city borders, making a centroid outside of the official city bounds the nearest centroid.
...
By visually examining property prices, we can note that they are related to proximity to the city center. The Wawer district also presents some high prices, likely stemming from its emptiness, enabling investors to construct bigger properties, or even luxury ones.
...
```
From the histograms above, we can see that squareMeters and price are similarily distributed, with a long right side tail. This gives us even more reason to combine the two, ensuring a distribution which resembles a normal one.
